...
def check_guess(message, lower_bound, upper_bound):
    """
    Checks whether the guess input by the user lies within the bounds of
    possible correct answers and asks for a new guess if this is not the case.
    """
...
def play_round(difficult=False, lives=5, lower_bound=0, upper_bound=20):
    ...
    guess = check_guess(
        L["guess_prompt_with_lives"].format(
            lower_bound=lower_bound,
            upper_bound=upper_bound,
            guesses=lives,
            guess_word=guess_word(lives),
        ),
        lower_bound,
        upper_bound,
    )
    ...
    while True:
        if guess == random_number:
            print(
                L["guess_response_success"].format(
                    guesses=total_lives + 1 - lives, guess_word=guess_word(lives)
                )
            )
            break
        ...
        hint = (
            ""
            if difficult
            else L["hint"].format(L["lower"] if guess > random_number else L["higher"])
        )

        guess = check_guess(
            L["guess_response_fail"].format(
                hint=hint, guesses=lives, guess_word=guess_word(lives)
            ),
            lower_bound,
            upper_bound,
        )
...
def get_args():
    ...
    parser.add_argument(
        "-d",
        "--difficult",
        help="Set game mode to difficult.",
        action="store_true",
    )
    parser.add_argument(
        "-l",
        "--lives",
        help="Set number of lives to play with.",
        type=int,
        default=5,
    )
    parser.add_argument(
        "-lb",
        "--upper-bound",
        help="Set the upper bound of the random integer number generator.",
        type=int,
        default=20,
    )
    parser.add_argument(
        "-ub",
        "--lower-bound",
        help="Set the lower bound of the random integer number generator.",
        type=int,
        default=0,
    )
    parser.add_argument(
...
def main():
    """
    Configures settings and runs play_game function.
    """
    global L
